...
# Guard against decompression bomb attacks: ~5000x5000 px
Image.MAX_IMAGE_PIXELS = 25_000_000


def _check_image_magic_bytes(data: bytes) -> bool:
    """Return True if the first bytes match a known image format signature."""
...
class StorageService:
    ...
    def validate_file_content(self, image_data: bytes) -> tuple[bool, str]:
        """Validate file content using magic-byte inspection.

        Rejects files whose bytes do not match a supported image header,
        even if their extension appears valid.

        Returns:
            Tuple of (is_valid, error_message)
        """
        if not _check_image_magic_bytes(image_data):
            return False, "File content does not match a supported image type."
        return True, ""
