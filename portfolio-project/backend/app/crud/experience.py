...
def _apply_experience_translation(
    ...
    The object is expunged from the session before mutation so that a
    later ``db.commit()`` cannot flush translated values back as the
    canonical English columns.
    """
...
def get_experiences(
    ...
    return [_apply_experience_translation(item, language, db) for item in experiences]
...
def get_experience_by_id(
    ...
    return _apply_experience_translation(experience, language, db)
...
def get_experiences_count(
    db: Session,
    experience_type: Optional[str] = None,
) -> int:
    """Get total count of experiences using a proper COUNT query."""
    query = db.query(func.count(Experience.id))
    if experience_type:
        query = query.filter(Experience.experience_type == experience_type)
