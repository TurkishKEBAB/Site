...
def _apply_blog_translation(
    ...
    The post is expunged from the session before mutation so that a later
    ``db.commit()`` (e.g. in the view-counter) cannot flush translated
    values back as the canonical English columns.
    """
    if db is not None:
        try:
            db.expunge(post)
...
def get_blog_posts(
    ...
    return [_apply_blog_translation(post, language, db) for post in posts]
...
def get_blog_post_by_slug(
    ...
    return _apply_blog_translation(post, language, db)
...
def increment_blog_views(db: Session, post_id: uuid.UUID) -> bool:
    ...
    Uses a single UPDATE statement to avoid read-modify-write race
...
def _escape_ilike(value: str) -> str:
    """Escape ILIKE special characters to prevent wildcard injection DoS."""
...
def search_blog_posts(
    ...
    """
    escaped = _escape_ilike(search_query)
    search = f"%{escaped}%"

    db_query = db.query(BlogPost).options(
        joinedload(BlogPost.translations),
