...
def get_contact_messages_count(db: Session, unread_only: bool = False) -> int:
    """Get total count of contact messages using a proper COUNT query."""
    query = db.query(func.count(ContactMessage.id))
    if unread_only:
        query = query.filter(ContactMessage.is_read == False)
