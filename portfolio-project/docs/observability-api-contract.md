...
Preferred release id is the deploy-provided git SHA:

1. Explicit `SENTRY_RELEASE` / `NEXT_PUBLIC_SENTRY_RELEASE`
2. `GITHUB_SHA`
3. `RAILWAY_GIT_COMMIT_SHA`
4. `VERCEL_GIT_COMMIT_SHA` / `NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA`
5. Backend `VERSION` or `NODE_ENV` fallback

This keeps backend and frontend events joinable by commit when both deployments expose the same SHA.
