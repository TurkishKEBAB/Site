...
- [ ] **Step 3: Run backend checks.** From `portfolio-project`, run `python -m pytest -q` and `python -m flake8 backend/app backend/tests --select=E9,F63,F7,F82 --count --statistics`; record the local Passlib/bcrypt environment failure separately rather than weakening gates.
